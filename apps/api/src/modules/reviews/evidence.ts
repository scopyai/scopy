...
const diffLinesByFile = (diffFiles: ParsedDiffFiles) => {
  const result = new Map<string, { changed: Set<number>; anchors: Set<number> }>()
...
const clampEvidenceWindow = ({
  ...
  const startLine = Math.max(1, Math.min(center - before, totalLines - maxLines + 1))
  return {
    startLine,
...
export const validateFindingEvidence = async ({
  ...
  if (!fileDiffLines) {
    hardReasons.push("File is not a changed head-side file in this pull request.")
  }
  ...
  if (hardReasons.length === 0 && totalLines) {
    try {
      ...
      // The original source read succeeded; keep that source in the artifact.
    }
  }
